...
def employees(request):
    ...
    return render(request, 'estate_agency/employees.html',
                  {"employees": employees})
...
def customers(request):
    ...
    return render(request, 'estate_agency/customers.html',
                  {"customers": customers})
...
def my_estates(request):
    estates = (Realty.objects.filter(owner=Owner.objects
    ...
    if sort_by:
        estates = estates.order_by(sort_by)
    logger.info(f"Returned user owned estates queryset with filter '{query}' and sorted by '{sort_by}' ")
    return render(request, "estate_agency/estates.html",
                  { "estates": estates,
                            "offers": len(estates)})
